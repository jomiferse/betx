...
Betfair API integration events are written to operational logs without request bodies, credentials, app keys, passwords, session tokens, or authentication headers. Use `betfair.session.login`, `betfair.session.cache_hit`, `betfair.session.keep_alive`, `betfair.api.request`, and `betfair.api.response` to confirm login reuse, keep-alive activity, JSON-RPC methods, results, attempts, and durations.
